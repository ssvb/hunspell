...
struct lzma2_dec {
	/* Position in hunxz_dec_lzma2_run(). */
	enum lzma2_seq {
		SEQ_CONTROL,
		SEQ_UNCOMPRESSED_1,
		SEQ_UNCOMPRESSED_2,
 ...
 */
static void dict_reset(struct dictionary *dict, struct hunxz_buf *b)
{
...
static void dict_uncompressed(struct dictionary *dict, struct hunxz_buf *b,
			      uint32_t *left)
{
	size_t copy_size;
 ...
 */
static uint32_t dict_flush(struct dictionary *dict, struct hunxz_buf *b)
{
 ...
 */
static bool rc_read_init(struct rc_dec *rc, struct hunxz_buf *b)
{
...
static uint16_t *lzma_literal_probs(struct hunxz_dec_lzma2 *s)
{
...
static void lzma_literal(struct hunxz_dec_lzma2 *s)
{
	uint16_t *probs;
	uint32_t symbol;
	uint32_t match_byte;
...
static void lzma_match(struct hunxz_dec_lzma2 *s, uint32_t pos_state)
{
	uint16_t *probs;
	uint32_t dist_slot;
	uint32_t limit;
 ...
 */
static void lzma_rep_match(struct hunxz_dec_lzma2 *s, uint32_t pos_state)
{
	uint32_t tmp;
...
/* LZMA decoder core */
static bool lzma_main(struct hunxz_dec_lzma2 *s)
{
	uint32_t pos_state;
 ...
 */
static void lzma_reset(struct hunxz_dec_lzma2 *s)
{
	uint16_t *probs;
	size_t i;
 ...
 */
static bool lzma_props(struct hunxz_dec_lzma2 *s, uint8_t props)
{
	if (props > (4 * 5 + 4) * 9 + 8)
		return false;
 ...
 */
static bool lzma2_lzma(struct hunxz_dec_lzma2 *s, struct hunxz_buf *b)
{
	size_t in_avail;
	uint32_t tmp;
 ...
 */
XZ_EXTERN enum hunxz_ret hunxz_dec_lzma2_run(struct hunxz_dec_lzma2 *s,
				       struct hunxz_buf *b)
{
	uint32_t tmp;
...
XZ_EXTERN struct hunxz_dec_lzma2 *hunxz_dec_lzma2_create(enum hunxz_mode mode,
						   uint32_t dict_max)
{
	struct hunxz_dec_lzma2 *s = kmalloc(sizeof(*s), GFP_KERNEL);
	if (s == NULL)
		return NULL;
...
XZ_EXTERN enum hunxz_ret hunxz_dec_lzma2_reset(struct hunxz_dec_lzma2 *s, uint8_t props)
{
...
XZ_EXTERN void hunxz_dec_lzma2_end(struct hunxz_dec_lzma2 *s)
{
	if (DEC_IS_MULTI(s->dict.mode))
		vfree(s->dict.buf);
...
struct hunxz_dec_microlzma {
	struct hunxz_dec_lzma2 s;
};

enum hunxz_ret hunxz_dec_microlzma_run(struct hunxz_dec_microlzma *s_ptr,
				 struct hunxz_buf *b)
{
	struct hunxz_dec_lzma2 *s = &s_ptr->s;
	...
	if (s->lzma2.sequence != SEQ_LZMA_RUN) {
		...
		/*
		 * hunxz_dec_microlzma_reset() doesn't validate the compressed
...
struct hunxz_dec_microlzma *hunxz_dec_microlzma_alloc(enum hunxz_mode mode,
						uint32_t dict_size)
{
	struct hunxz_dec_microlzma *s;
...
void hunxz_dec_microlzma_reset(struct hunxz_dec_microlzma *s, uint32_t comp_size,
			    uint32_t uncomp_size, int uncomp_size_is_exact)
{
	/*
	 * comp_size is validated in hunxz_dec_microlzma_run().
...
void hunxz_dec_microlzma_end(struct hunxz_dec_microlzma *s)
{
